...
namespace ShoppingStore.API.DbContexts
{
    public class ShoppingStoreContext : DbContext
    {
        ...
        {
            modelBuilder.ApplyUtcDateTimeConverter();//Put before seed data and after model creation (fix time must be UTC PostgresDb)

            var tempCurrentDate = new DateTime(2024, 10, 19, 22, 42, 59, DateTimeKind.Utc).AddTicks(1312);
